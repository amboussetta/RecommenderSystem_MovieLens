...
This model will be formulated as: $$ Y_{u,i} = \mu + b_i +b_u + \epsilon_{u,i} $$
...
```
As expected, all the models are better than the one value prediction based on the average. The movieId variable had a strong impact on the rmse and combining it with the usedId made the rmse smaller. The age variable a very small impact comparing to the first two. The best rmse `r model_4_rmse is then obtained by the last model 
